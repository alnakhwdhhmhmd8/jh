...
@app.on_message(filters.regex(r'حساب العمر'), group=7568)
async def calculate_age(client, message):
    try:
        birth_date = datetime.datetime.strptime(message.text.split(" ")[2], "%d-%m-%Y")
    except:
        await message.reply_text("خطأ عزيزي برجاء ادخال تاريخ ميلادك بهذه الصيغه مثال - (25-12-2004 )")
        return
