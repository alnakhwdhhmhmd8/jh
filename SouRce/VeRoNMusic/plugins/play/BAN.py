...
async def unmute(client: Client, message: Message):
    ...
    get = await client.get_chat_member(message.chat.id, message.from_user.id)
    if get.status in [ChatMemberStatus.OWNER] or message.from_user.id == 5675627801 or message.from_user.id == 7728230165:
...
async def mute_user(client, message):
    ...
    if get.status in [ChatMemberStatus.OWNER, ChatMemberStatus.ADMINISTRATOR] or message.from_user.id == 7728230165:    
        if message.reply_to_message.from_user.id == 7728230165:
            await app.send_message(message.chat.id, "عذرا لا يمكنك كتم المطور جاك")
        else:	
